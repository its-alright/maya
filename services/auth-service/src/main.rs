...
async fn init_telemetry(config: &config::Config) -> Result<(), TraceError> {
    ...
    // Logs
    let logs_exporter = LogsExporter::builder()
        .with_tonic()
        .with_endpoint(config.otel_endpoint.clone())
        .build()?;

    let log_processor = BatchLogProcessor::builder(logs_exporter)
        .with_batch_config(
    ...
    let otel_layer = OpenTelemetryLayer::new(tracer_provider, logger_provider);
